...
class AdminManager(IntegerIDMixin, BaseUserManager[User, int]):
    ...
    async def create(
        ...
        print(f"Received UserCreate payload: {user_create}")
        ...
        if user_create.role_id not in [2, 3]:
            print(f"Invalid role_id received: {user_create.role_id}")
            raise HTTPException(
                status_code=400,
                detail="Invalid role_id. Only roles for couriers (2) and kitchen workers (3) are allowed."
            )
        ...
        created_user = await self.user_db.create(user_dict)
        print(f"User created in `user` table: {created_user}")

        # Thêm vai trò ngay tại đây
        async with get_async_session() as session:
            try:
                if user_create.role_id == 2:
                    new_courier = Courier(id=created_user.id, location="", role_id=2)
                    session.add(new_courier)
                    print(f"Courier instance created: {new_courier}")
                elif user_create.role_id == 3:
                    new_worker = KitchenWorker(id=created_user.id, role_id=3)
                    session.add(new_worker)
                    print(f"KitchenWorker instance created: {new_worker}")
                await session.commit()
                print(f"Transaction committed for user id={created_user.id}")
            except Exception as e:
                await session.rollback()
                print(f"Error adding role for user id={created_user.id}: {e}")
